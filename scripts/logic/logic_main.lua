...
final_fantasy_tactics_ivalice_island_location = {}
final_fantasy_tactics_ivalice_island_location.__index = final_fantasy_tactics_ivalice_island_location
...
function final_fantasy_tactics_ivalice_island_location.new(name)
    local self = setmetatable({}, final_fantasy_tactics_ivalice_island_location)
    if name then
...
function final_fantasy_tactics_ivalice_island_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = final_fantasy_tactics_ivalice_island_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function final_fantasy_tactics_ivalice_island_location:connect_two_ways(exit, rule)
...
function final_fantasy_tactics_ivalice_island_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function final_fantasy_tactics_ivalice_island_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function final_fantasy_tactics_ivalice_island_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function final_fantasy_tactics_ivalice_island_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function final_fantasy_tactics_ivalice_island_location:accessibility()
...
-- 
function final_fantasy_tactics_ivalice_island_location:discover(accessibility, keys)
...
entry_point = final_fantasy_tactics_ivalice_island_location.new("entry_point")
